...
performance, energy efficiency, and reliability. AI 
applications are highly memory-intensive and require new supports
from memory architecture~\cite{TechReview:Online-2026}.
Enormous research efforts have been spent to tackle the memory wall
problem at different memory hierarchy levels.  Memory
systems are increasingly complicated, with multiple levels of caches
and high data-rate DDR5 devices in main-stream systems, and potentially
new and diverse memory technologies in the near future.
As a result, the management of memory system
is also getting more and more complicated in order to improve
...
rates~\cite{Das:TACO-2015, Jaleel:ISCA-2010, Sethumurugan:HPCA-2021,
Shi:MICRO-2019, Wu:TACO-2011}; 
numerous prefetching schemes have been proposed to predict
future cache misses and fetch those in advance~\cite{Ai:MICRO-2025,
Ainsworth:ISCA-2024, Bakhshalipour:HPCA-2018, Bakhshalipour:HPCA-2019,
Bera:MICRO-2021, Bera:HPCA-2026, Bera:MICRO-2019, Chen:HPCA-2025,
Duong:ISCA-2024, He:MICRO-2025}; 
...
their bandwidth~\cite{Rixner:ISCA-2000, Kim:HPCA-2010, Kim:MICRO-2010,
Mutlu:MICRO-2007, Mutlu:ISCA-2008, Shao:HPCA-2007, Subramanian:TPDS-2016}.

Although each component of those types can work
effectively to achieve its own goal, they usually operate in isolation
without any explicit coordination in the same system.  For example,
a replacement policy may not consider its impact on the prefetching
engine and the memory access scheduler when making the replacement decision.
...
In this project, we propose a coordinated cache and memory management framework
via efficient and elastic machine learning techniques.

a machine-learning based, coordinated cache and memory management framework that
coordinates the management policies at different memory hierarchy levels to
achieve the optimal memory performance, energy efficiency and reliability.  Our
proposed schemes have two steps. The first one is an enhancement to existing
systems. It utilizes existing memory management schemes, such as the existing
cache replacement policies or prefetching policies, but add a helper component
at each level to improve the existing policy's efficiency, such as a prefetching
filter to filter out some unnecessary prefetching requests.  The helpers will
use the feedback information from other levels to coordinate the memory
management.  This approach can limit the design complexity since it is an
enhancement to existing schemes.  The second approach will go one step further
and re-design the management schemes at each level by considering the status of
other levels.  This would lead to better performance and energy efficiency but
also higher design complexity and implementation cost.
...
improve the performance and energy efficiency.  It used to be the isolation
...
feedback information from other levels can be used as features of the
...
Of course, all these interactions between different memory hierarchy
...
part of our study.  Furthermore, since the complexity of coordinated
